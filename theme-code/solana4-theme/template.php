...
if (theme_get_setting('solana4_rebuild_registry')) {
  drupal_rebuild_theme_registry();
}

// Add Zen Tabs styles
if (theme_get_setting('solana4_zen_tabs')) {
  drupal_add_css( drupal_get_path('theme', 'solana4') .'/css/tabs.css', 'theme', 'screen');
}
...
function solana4_preprocess_page(&$vars, $hook) {
  ...
  if (user_access('administer blocks')) {
	...
	if (theme_get_setting('solana4_wireframe')) {
  ...
  if (!$vars['is_front']) {
    ...
    $body_classes[] = solana4_id_safe('page-'. $path);
    $body_classes[] = solana4_id_safe('section-'. $section);
...
function solana4_preprocess_node(&$vars, $hook) {
  ...
  $classes[] = solana4_id_safe('node-type-' . $vars['type']);
...
function solana4_preprocess_block(&$vars, $hook) {
    $block = $vars['block'];

    // special block classes
    $classes = array('block');
    $classes[] = solana4_id_safe('block-' . $vars['block']->module);
    $classes[] = solana4_id_safe('block-' . $vars['block']->region);
    $classes[] = solana4_id_safe('block-id-' . $vars['block']->bid);
    $classes[] = 'clearfix';
    ...
    if (theme_get_setting('solana4_block_editing') && user_access('administer blocks')) {
...
function solana4_preprocess_comment(&$vars, $hook) {
...
function solana4_menu_item_link($link) {
...
function solana4_menu_local_tasks() {
  $output = '';
...
function solana4_menu_item($link, $has_children, $menu = '', $in_active_trail = FALSE, $extra_class = NULL) {
...
#New line added to get unique classes for each menu item
  $css_class = solana4_id_safe(str_replace(' ', '_', strip_tags($link)));
...
function solana4_id_safe($string) {
...
function solana4_breadcrumb($breadcrumb) {
  if (!empty($breadcrumb)) {
...
*
*/
function solana4_links($links, $attributes = array('class' => 'links')) {
  $output = '';
...
/**
 * Theme function for modifying default nodereference field formatter.
 */
function solana4_nodereference_formatter_default($element) {
  $output = '';
  if (!empty($element['#item']['safe']['nid'])) {
    $output = "Click here to register for the " . l($element['#item']['safe']['title'], 'node/' . $element['#item']['safe']['nid']);
